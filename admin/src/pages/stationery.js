...
const ProductGrid = () => {
  ...
  const fetchProducts = useCallback(async (filter,term, currPage, productsPerPage = parseInt(windowWidth / 180 * 4)) => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/product?${queryParams.toString()}`, {
              headers: {
  ...
  //       const imgResponse = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/product/${product._id}/image`, {
  //         responseType: 'blob',
  //         headers: {
  //           Authorization: `Bearer ${token}`
  //         }
